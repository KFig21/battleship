...
function App() {
  ...
  const [playerBoard, setPlayerBoard] = useState([]);
    ...
    const [cpuShipToPlace, setCpuShipToPlace] = useState(CpuShipData[cpuShipToPlaceIndex]);
  ...
  const cpuTurn = () => {
    ...
    if (!onTarget){
      ...
      if(preferredTargets.length > 0){
        ...
        while(playerBoard.includes(attack) && preferredTargets.length > 0){
          ...
          attack = preferredTargets[preferredIndex];
          // if there are no valid preferred targets it sets the attack as a valid random target and proceeds through the conditional
          if(preferredTargets.length === 0) attack = findRandomTarget();
        }
      } else {
    ...
    setPlayerBoard((currentBoard) => [...currentBoard, attack])
      for(let i = 0; i < ships.length; i++){
        for(let j = 0; j < ships[i].coords.length; j++){
            if (ships[i].coords[j] === attack){
                document.getElementById(`Player-${attack}`).style.backgroundColor = "var(--alertRed)";
                ships[i].health --;
                ...
                if(ships[i].health === 0){
                    ...
                    for(let k = 0; k < ships[i].size; k++){
                        document.getElementById(`Player-${ships[i].coords[k]}`).style.backgroundColor = "darkred";
                    }
                }
      ...
      document.getElementById(`Player-${attack}`).style.backgroundColor = "var(--miss)"
      switchPlayer();
      return
  }
  ...
  const placeShip = (coords) => {
    ...
    if (validateShip(coordinates, "Player")){
      ...
      for(let i = 0; i < coordinates.length; i++){
        ...
        document.getElementById(`Player-${coordId}`).style.backgroundColor = "var(--ship)";
      }
    ...
    <div className="App">
      <Header />
        ...
        <div className="lower-container">
          {/* <ShipYard /> */}
          {!gameStarted &&(
            <button className="orientation-button" onClick={changeAxis}>Set {axis}</button>
          )}
          {gameWon &&(
            <button className="orientation-button" onClick={startGame}>Play Again</button>
          )}
        </div>
    </div>
  );
